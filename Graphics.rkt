...
(define (make-window w h title)
  ...
  (let* ((frame-done #t)
    ...
    (define (paint-callback canvas dc)
      ...
      ; construct the fps string. 
      (define fps-string (construct-fps-string fps))
      ; If show-fps, set the fps in the frame label.
      (when show-fps (send frame set-label (string-append title fps-string)))
      ; Make sure the drawing-loop only gets called again when the drawing is done.
      (set! frame-done #t))
    ...
    (define (construct-fps-string fps)
    ...
    (define frame (new frame%
                       [label title]
                       [width w]
                       [height (+ h 30)])) ;incalculate border
    ...
    (define (launch-game-loop)
      (let* ((max-fps 500)          ; The maximum frames per seconds someone can get is 500.
        ...
        (define (game-loop)  
          ...
          ; If the game-loop is not done we immediately fire it again but first
          ; we wait for min-delta-time, which is typically the min-wait-per-frame
          (when  (and frame-done (>= delta-time ms-per-frame))
            ; reset frame-done
            (set! frame-done #f)
          ...
          ; after waiting min-delta-time ms.
          (send game-loop-timer start (calculate-interval)))
        
        ; a timer drives the game-loop which calls teh game-loop after waiting
  ...
  (let ((timer (new timer% [notify-callback callback] [interval interval] [just-once? #f])))
    (define (dispatch msg)
      (cond ((eq? msg 'start) (send timer start interval))
            ...
            ((eq? msg 'timer) timer)
            (else (display "wrong message sent to timer: ") (display msg) (newline))))
    dispatch))
    ...
    (define (draw dc)    
      ...
      (if mask
         (begin (send rotated-mask-dc draw-bitmap mask 0 0)
                (send dc draw-bitmap rotated-bitmap x y 'solid transparent rotated-mask))
  ...
  (let ((index 0) 
    ...
    (define (dispatch msg )    
      (cond 
            ...
            ((eq? msg 'draw-line) draw-line) 
            ((eq? msg 'draw-text) draw-text) 
            (else (display "wrong message sent to tile-sequence: " ) (display msg) (newline))))
    dispatch))
